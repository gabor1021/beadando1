...
def beolvas():
    file = open("beadando.txt", 'r+', encoding='utf8')
    varos = []
    for i in file:
        adat = []
        i = i.strip().split("   ")
...
class Varos:
    ...
    def print_all(self):
        try:
            print(self.nev, self.country, self.continent, self.lat, self.lon, self.travel_time, self.transport,
                  sep=", ")
        except(AttributeError):
            try:
                print(self.nev, self.country, self.lat, self.lon, self.travel_time, self.transport,
                      sep=", ")
            except(AttributeError):
                print(self.nev, self.lat, self.lon, self.travel_time, self.transport,
                      sep=", ")
    ...
    def file_write_tavolsag_km(self):
        file = open("tavolsagok.txt", 'w', encoding="utf8")
        print("Távolság:\n\t%f km" % self.tavolsag())
        file.write("Pécs - %s: \n\t%f km \n\t%f mérföld" % (self.nev, self.tavolsag(), self.mile_convert()))
...
class Europai_varos(Varos):
    def __init__(self, nev: str, country: str, lat: float, lon: float, travel_time: int, transport: int, timezone: int):
    ...
    def ido_calc(self):
        if self.transport == "Autó":
            ...
            print("Utazási idő: %d:%d:%d repülővel, %d:%d:%d autóval" % (
                plane_days, plane_hours, plane_minutes, car_days, car_hours, car_minutes))


class Masik_kontinens_varos(Europai_varos):
    def __init__(self, nev: str, country: str, continent: str, lat: float, lon: float,
                 travel_time: int, transport: int, timezone: int):
        super().__init__(nev, country, lat, lon, travel_time, transport, timezone)
    ...
    def file_write_time(self):
        file = open("tavolsagok.txt", "w")
        ido = self.time_calc()
        file.write("Ország: %s, Kontinens: %s"%(self.melyik_orszag(),self.melyik_kontinens()))
        file.write("\n\tUtazási idő: %d:%d:%d" % (ido[0], ido[1], ido[2]))
    ...
    for i in beolvas():
        if i[0] == "Pécs":
            ...
            globals()[i[0]] = Europai_varos(f"{i[0]}", f"{i[1]}", i[3], i[4], i[5],
                                            vehicle.car.value, i[6])
        elif i[2] == "-":
            globals()[i[0]] = Europai_varos(f"{i[0]}", f"{i[1]}", i[3], i[4], i[5],
                                            vehicle.car_or_plane.value, i[6])
        else:
            globals()[i[0]] = Masik_kontinens_varos(f"{i[0]}", f"{i[1]}", f"{i[2]}", i[3], i[4], i[5],
                                                    vehicle.plane.value, i[6])
